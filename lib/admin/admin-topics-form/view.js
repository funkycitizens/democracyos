...
export default class TopicForm extends FormView {
  ...
  postserialize (data) {
    ...
    data.showAbstain = data.showAbstain || false
    data.yeaLabel = data.yeaLabel || t('proposal-options.yea')
    data.nayLabel = data.nayLabel || t('proposal-options.nay')
    data.abstainLabel = data.abstainLabel || t('proposal-options.abstain')
  ...
  renderToggles () {
    ...
    var toggle = new Toggle()
    toggle.label('Yes', 'No')
    toggle.name('showAbstain')
    toggle.value(this.topic && this.topic.showAbstain !== false)
    this.find('.showAbstain-toggle').append(toggle.el)
  }
